...
class GitHubAnalyzer:
    ...
    def analyze_repository(self, repo_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[*] Encontrados {len(java_files)} arquivos .java no repositorio")
            ...
            if len(java_files) < 100:
                print(
                    f"[!] Repositorio com menos de 100 arquivos Java ({len(java_files)} encontrados)"
                )
                print(
                    f"[!] Provavelmente e um repositorio de exemplos/tutoriais. Pulando analise."
                )
                return result

            # 3. Executa CK
            output_dir = os.path.join(TEMP_DIR, f"ck_output_{repo.replace('/', '_')}")
